...
int8_t W5500_TCP_Socket_Open(W5500_Config *config, W5500_Socket_Config *socket_config)
{
	...
	Write_1_Byte(config, W5500_Socket_Register.Sn_MR, W5500_Control_Register.Socket_0_Control, W5500_Socket_Mode.TCP);
	while(counter_tcp < counter_overflow_tcp)
	{
	...
	if(counter_tcp >= counter_overflow_tcp){
		return -1;
	}
...
/********************************** Connect *******************************************************/
		...
		while(counter_tcp < counter_overflow_tcp)
		{
			...
			Write_1_Byte(config, W5500_Socket_Register.Sn_CR, W5500_Control_Register.Socket_0_Control, 0x04);
			Read_1_Byte(config, W5500_Socket_Register.Sn_SR, W5500_Control_Register.Socket_0_Control, &socket_response);
			if(socket_response == W5500_Socket_Status.SOCK_ESTABLISHED)
			{
				break;
			}
			counter_tcp ++;
...
//	RX_WR_TOTAL = (RX_WR[1] << 8) | (RX_WR[0] << 0);
	...
	if(counter_tcp >= counter_overflow_tcp) return -3;
